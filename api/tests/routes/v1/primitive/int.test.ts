...
const intEndpoint = '/api/v1/primitive/int'

describe(`${intEndpoint}`, () => {
  test('returns 400 when max is too large', async () => {
    const response = await supertest(app)
      .get(`${intEndpoint}?min=0&max=9007199254740992`)
  ...
  test('returns 400 when min is too large', async () => {
    const response = await supertest(app)
      .get(`${intEndpoint}?min=9007199254740992&max=9007199254740993`)
  ...
  test('returns 400 when min is not a number', async () => {
    const response = await supertest(app)
      .get(`${intEndpoint}?min=hello&max=5`)
  ...
  test('returns 400 when max is not a number', async () => {
    const response = await supertest(app)
      .get(`${intEndpoint}?min=5&max=hello`)
